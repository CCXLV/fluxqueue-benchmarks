...
is_multithreaded() {
    ...
    if [ -f "/proc/$pid/status" ]; then
        ...
        [ "$threads" -gt 1 ] && return 0
    fi
    return 1
}

# Get initial CPU times
for pid in $(cat pids.log); do
